...
NOTIFICATION_WORKER_REPLICAS="${NOTIFICATION_WORKER_REPLICAS:-1}"
...
TENANT_DIR="$PROVIDER_ROOT/envs/tenants/$TENANT_NAME"
...
export BACKEND_REPLICAS FRONTEND_REPLICAS WORKER_REPLICAS NOTIFICATION_WORKER_REPLICAS NGINX_REPLICAS REVERB_REPLICAS
...
if [[ "$DRY_RUN" == true ]]; then
  log "Dry run: docker stack deploy -c $PROVIDER_ROOT/templates/tenant-stack.yml $STACK_NAME"
  exit 0
fi

docker stack deploy --with-registry-auth -c "$PROVIDER_ROOT/templates/tenant-stack.yml" "$STACK_NAME"
log "Tenant stack deploy submitted: $STACK_NAME"

if [[ "$SKIP_MIGRATE" != true ]]; then
  log "Waiting before migration job"
  sleep 30
  docker run --rm \
    --network "${STACK_NAME}_tenant-internal" \
